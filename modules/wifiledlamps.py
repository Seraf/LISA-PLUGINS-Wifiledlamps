...
_ = translation = gettext.translation(domain='wifiledslamp', localedir=path, languages=[configuration['lang']]).ugettext
...
class Wifiledslamp:
    def __init__(self):
        ...
        self.configuration = mongo.lisa.plugins.find_one({"name": "Wifiledslamp"})
    ...
    def switch(self, jsonInput):
        if jsonInput['outcome']['entities']['wifiledslamp_actions']['value'] == 'on':
            self.led_connection.rgb.all_on()
            body = _('lights are on')
        elif jsonInput['outcome']['entities']['wifiledslamp_actions']['value'] == 'off':
            self.led_connection.rgb.all_off()
            body = _('lights are off')
        return {"plugin": "Wifiledslamp",
                "method": "switch",
                "body": body
        }
